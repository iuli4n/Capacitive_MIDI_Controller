/*
This is a MIDI controller built on 8 capacitive sensing pads.
...
const int CAPTHRESHOLD = 500;

const int DEBUGTHIS = -1;
...
// BUTTONS
const int NButtons = NCaps; //***  total number of push buttons
//const int buttonPin[NButtons] = {8, 3, 5, 7}; //*** define digital pins connected from button to Arduino; (ie {10, 16, 14, 15, 6, 7, 8, 9, 2, 3, 4, 5}; 12 buttons)
...
void caps() {
  ...
  for (int i = 0; i <NCaps ; i++) {
    ...
    int val = cs[i].capacitiveSensor(30);
    int state = (val < CAPTHRESHOLD);

    if (DEBUGTHIS == i) Serial.println(val);
    
    buttonCState[i] = state;
    ...
    if ((millis() - lastDebounceTime[i]) > debounceDelay) {
      ...
      if (buttonPState[i] != buttonCState[i]) {
        ...
        if (buttonCState[i] == LOW) {
          ...
          if (DEBUGTHIS<0) MIDI.sendNoteOn(note + i, 127, midiCh); // note, velocity, channel
          ...
          if (DEBUGTHIS<0) MIDI.sendNoteOn(note + i, 0, midiCh); // note, velocity, channel
...
void buttons__OLD() {
  ...
  for (int i = 0; i < NButtons; i++) {
    ...
    int state = 0;//digitalRead(buttonPin[i]);  // read pins from arduino

    const int CAPTHRESHOLD = 300;
    
    if (i == 0) {
      state = (cs[0].capacitiveSensor(30) < CAPTHRESHOLD);
      //Serial.println(cap1);
    }
    if (i == 2) {
      state = (cs[1].capacitiveSensor(30) < CAPTHRESHOLD);
      //Serial.println(cap1);
    }
    if (i == 3) {
      state = (cs[2].capacitiveSensor(30) < CAPTHRESHOLD);
      //Serial.println(cap1);
    }
